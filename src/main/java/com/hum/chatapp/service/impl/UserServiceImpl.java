...
public class UserServiceImpl implements UserService {
     ...
     */
    @Override
    public ResponseEntity<ApiResponse> saveUser(Chatuser user) {
    ...
    public ResponseEntity<ApiResponse> findUserByEmail(String email) {
        Chatuser user = userDao.findUserByEmail(email);
        return new ResponseEntity<>(
                ApiResponse.builder()
                        .statusCode(200)
                        .status("Success")
    ...
    public ResponseEntity<ApiResponse> findAllUsers() {
        List<Chatuser> list = userDao.findAllUsers();
        return new ResponseEntity<>(
                ApiResponse.builder()
                        .statusCode(200)
                        .status("Success")
    ...
    public ResponseEntity<ApiResponse> findAllUsersExceptThisUserId(int userId) {
        List<Chatuser> list = userDao.findAllUsersExceptThisUserId(userId);
        return new ResponseEntity<>(
                ApiResponse.builder()
                        .statusCode(200)
                        .status("Success")
