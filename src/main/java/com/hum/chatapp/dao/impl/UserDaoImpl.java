...
import com.hum.chatapp.entity.Chatuser;
...
public class UserDaoImpl implements UserDao {
    ...
    public Chatuser saveUser(Chatuser user) {
        try {
            pst = db.get().prepareStatement("INSERT INTO chatuser (first_name, last_name, email) VALUES (?, ?, ?)", Statement.RETURN_GENERATED_KEYS);
    ...
    public Chatuser findUserByEmail(String email) {
        try {
            pst = db.get().prepareStatement("SELECT user_id, first_name, last_name, email FROM chatuser WHERE email = ?");
            ...
            if (rs.next()) {
                ...
                throw new UserNotFoundException("UserNotFoundException: " + email);
            }
     ...
     * @throws InternalServerErrorException if there is an internal server error.
     */
    public List<Chatuser> findAllUsersExceptThisUserId(int userId) {
        try {
            List<Chatuser> list = new ArrayList<>();
            pst = db.get().prepareStatement("SELECT user_id, first_name, last_name, email FROM chatuser WHERE user_id != ?");
            pst.setInt(1, userId);
            ResultSet rs = pst.executeQuery();
            Chatuser user;
            while (rs.next()) {
